...
function require(id){if(cache[id])return cache[id].exports;if(!modules[id])throw new Error('QR module not found: '+id);var m=cache[id]={exports:{}};modules[id](m,m.exports,require);return m.exports;}
var QRCode=require('./index'), Levels=require('./QRErrorCorrectLevel');

function create(text, level){
  var qr=new QRCode(-1, Levels[level||'L'] == null ? Levels.L : Levels[level||'L']);
...
function renderCanvas(canvas,text,options){
  options=options||{};
  var qr=create(text,options.level||'L'), count=qr.getModuleCount();
  var size=Math.max(180,Number(options.size)||320), margin=Math.max(2,Number(options.margin)||4);
  var scale=Math.max(1,Math.floor(size/(count+margin*2)));
  var actual=(count+margin*2)*scale;
  canvas.width=actual;canvas.height=actual;
  var ctx=canvas.getContext('2d');ctx.imageSmoothingEnabled=false;
  ctx.fillStyle=options.light||'#ffffff';ctx.fillRect(0,0,actual,actual);
  ctx.fillStyle=options.dark||'#07131f';
  for(var r=0;r<count;r++)for(var c=0;c<count;c++)if(qr.isDark(r,c))ctx.fillRect((c+margin)*scale,(r+margin)*scale,scale,scale);
  return {moduleCount:count,size:actual,canvas:canvas};
}
function toDataURL(text,options){var c=document.createElement('canvas');renderCanvas(c,text,options);return c.toDataURL('image/png');}
root.JB3DQRCode={create:create,renderCanvas:renderCanvas,toDataURL:toDataURL,Levels:Levels};
